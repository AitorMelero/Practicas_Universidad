 ...
 * Function: Programa que cifra o descifra un fichero con DES en CBC. 
 * ***********************************************************/
...
void muestra_info_parametros() {
    ...
    printf("./desCBC -C/D -k <clave> -iv <iv> -i <fichero entrada> -o <fichero salida>\n");
    printf("################################################\n");
    return;
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 10) {
        if (strlen(argv[3]) != 16 || strlen(argv[5]) != 8) {
            printf("La clave y el iv deben ser de 8 cifras (64 bits)!!!\n");
            return -1;
        }
    } else {
        if (strlen(argv[3]) != 8) {
    ...
    if (strcmp("-C", argv[1]) == 0) {
        ...
        clave_aux = (unsigned char*)calloc(9, sizeof(unsigned char));        
        clave = genera_clave();
        ...
        clave_aux = (unsigned char*)calloc(17, sizeof(unsigned char));
    ...
    if (descifra == FALSE) {
        ...
        texto_cifrado = cifrar_CBC(clave, iv, texto_plano, tam_i);
        ...
        texto_plano = descifrar_CBC(clave, iv, texto_cifrado, tam_i);
        if (texto_plano == NULL) {
