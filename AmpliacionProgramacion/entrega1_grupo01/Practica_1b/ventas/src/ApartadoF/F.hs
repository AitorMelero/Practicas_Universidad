{--
	...
	Fecha: 21/10/2020
	Funcionalidad: Implementacion del apartado 'f' de P1b.
--}
{-# LANGUAGE RankNTypes, GADTs, FlexibleInstances, TypeSynonymInstances, KindSignatures #-}

module ApartadoF.F where
...
precioFactura (FacturaUnitaria v) = precio v
precioFactura (FacturaMultiple [v]) = precio v
precioFactura (FacturaMultiple (v:vs)) = foldr (+) 0 [precio p | p <- v:vs]
...
(FacturaUnitaria v) $# a
            | sacarArticuloVenta v == nombre a = precio v
            | otherwise = 0
(FacturaMultiple [v]) $# a
            | sacarArticuloVenta v == nombre a = precio v
            | otherwise = 0
(FacturaMultiple (v:vs)) $# a = foldr (+) 0 [precio p | p <- v:vs, sacarArticuloVenta p == nombre a]
...
-- Main
mainApartadoF :: IO ()
mainApartadoF = do let a1 = Articulo 1 "Tornillo" 3.4
